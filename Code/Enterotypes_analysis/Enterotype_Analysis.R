...
matched_nodes_diff |> merge(w0_Sequences_nodes, by.x = "node", by.y = "Node_list") |>
  ...
  summarise(
    ...
    count = n()# Sum of non-absolute values of diff
  ) |>  arrange(desc(abs_Bact2_Bact2_minus_Bact2_other_diff_in_diff))|>   write.xlsx(file = "Analysis_Enterotype.xlsx", sheetName = "Averaged diff in diff per Family", append = T)
